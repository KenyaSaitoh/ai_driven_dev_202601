...
### 8.4 REST APIプロジェクトの開発前提

本標準はREST APIプロジェクトを対象とする:

* 画面（UI）は含まない。View/XHTMLに関するタスクは生成しない
* E2EテストはREST Assured等のHTTPクライアントで実施する
* JWT認証・CORS・HTTPステータスコードは本ドキュメントおよび[security.md](security.md)に従う
...
* セキュリティ: 認証・認可方式、JWT詳細、入力検証、ログマスキング、SQLインジェクション対策、パスワードハッシュ
* パフォーマンス: 最大件数・ページネーション、キャッシュ、外部APIタイムアウト・リトライ、N+1回避、非同期
* データ整合性・トランザクション: トランザクション境界、楽観/悲観ロック、論理/物理削除、分散トランザクション、外部API障害時の扱い

各項目の推奨実装・選択肢は本ドキュメントの該当セクション（5 セキュリティ、6 トランザクション、10 パフォーマンス）を参照し、詳細設計時に選択肢を提示してユーザーに判断を仰ぐ。
