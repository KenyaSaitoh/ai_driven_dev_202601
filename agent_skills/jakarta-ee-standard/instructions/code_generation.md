...
1. **最優先**: `@agent_skills/jakarta-ee-standard/principles/` 配下の共通憲章ファイルでJakarta EE開発の原則、アーキテクチャ方針、品質基準を確認。プロジェクト固有の憲章がある場合は `{project_root}/principles/` も併せて確認
...
#### 憲章の遵守
`@agent_skills/jakarta-ee-standard/principles/` 配下の共通憲章に記載された開発原則を全ての実装で遵守
- テストカバレッジ基準、アーキテクチャパターン、コーディング規約に従う
- 品質基準、セキュリティ要件、パフォーマンス基準を満たす
- プロジェクト固有の憲章（`{project_root}/principles/`）がある場合は、それも併せて遵守

#### 仕様書修正の制約
コード生成時における仕様書の修正には厳格な制約があります
...
- 実装詳細の調整が必要な場合は`detailed_design.md`で対応
- 上位仕様との矛盾を発見した場合は、実装を停止しユーザーに報告
...
このスキルは、仕様書（`{project_root}/specs/baseline/system/architecture_design.md`）に記載されたアーキテクチャパターンに自動的に適応します。
...
**実装の特徴**:
- 全エンティティ（Book、Stock、Category、Publisher等）を実装
- 全Dao、Service、Resourceを実装
- CORS設定でクロスオリジンリクエスト対応
- 楽観的ロック（@Version）による並行制御
...
**実装の特徴**:
- **プロキシパターン**: 特定のResource（例: BookResource、CategoryResource）は外部APIへ透過的転送のみ実装
- **独自実装パターン**: 特定のResource（例: AuthenResource、OrderResource）はビジネスロジックを実装
- **エンティティ制限**: 一部のエンティティ（例: 注文関連）のみ実装、他は外部API管理
- **外部API連携**: RestClientを実装（例: BackOfficeRestClient、CustomerHubRestClient）
- **JWT認証**: JwtUtil、JwtAuthenFilter、AuthenContext等の認証基盤を実装

**データ管理の制約**:
- functional_design.mdとexternal_interface.mdを参照して、どのエンティティを実装するか判断
- 実装しないエンティティ（外部API管理）は、DTO（TO）として定義のみ
