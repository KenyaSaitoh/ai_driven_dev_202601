...
このインストラクションは、完成したSPECから複数人が並行して作業できる実装タスクリストを分解・生成するためのものです。

重要な方針:
* タスクリストは抽象度の高いレベルで作成します
...
* architecture_design.md - 技術スタック、アーキテクチャパターン、ライブラリを確認
  * アーキテクチャパターンを識別: マイクロサービス、BFF、ハイブリッド等
  * データ管理方針を確認: 全エンティティ実装、一部エンティティ実装、外部API管理等
  * 外部連携要件を確認: REST API統合、プロキシパターン、独自実装パターン等
...
* エンティティ: architecture_design.mdとdata_model.mdから実装対象を判断
  * マイクロサービスの場合: 全エンティティを実装
  * BFFの場合: 一部エンティティ（例: 注文関連）のみ実装、他は外部API管理
* Dao: 実装するエンティティに対応するDaoを作成
  * 検索パターン: JPQL検索、Criteria API検索（仕様書に記載がある場合）
* 共通サービス: 複数APIで使用されるサービス
* 認証基盤: JWT認証（JwtUtil、JwtAuthenFilter、AuthenContext等）
* 外部API連携: RestClient（例: BackOfficeRestClient、CustomerHubRestClient）
* 共通DTO: ErrorResponse、外部API用DTO（BookTO、StockTO等）
* 共通ユーティリティ: MessageUtil等
* フィルター/例外ハンドラ: CorsFilter、OptimisticLockExceptionMapper等

* 重要: 共通コンポーネントの具体的な内容は、architecture_design.md、functional_design.md、external_interface.mdから判断してください。
...
* タスクID命名規則:
  * タスクIDはアンダースコア区切りを使用します（例: `T_SETUP_001`, `T_API001_003`）
...
* アーキテクチャパターンに応じた要件:
  * マイクロサービス: 全エンティティ、全Dao、楽観的ロック、2種類の検索（JPQL/Criteria API）、CORS対応
  * BFF: プロキシパターン、独自実装パターン、外部API連携クライアント、JWT認証、一部エンティティのみ

---

## 7. 生成手順

1. 憲章とSPEC分析: `@agent_skills/jakarta-ee-standard/principles/` 配下の共通憲章と全SPECファイル（system/とapi/）を読み込み、開発原則と機能全体を把握
2. アーキテクチャ識別: architecture_design.mdからアーキテクチャパターン（マイクロサービス、BFF等）を識別
3. 機能（API）抽出: 実装が必要なAPIを抽出し、依存関係と共通コンポーネントを識別
4. タスク分割: API数に応じた適切なファイル分割方法を決定（小規模: 少数ファイル、中規模: API別、大規模: グループ別）
5. タスク構成: 各SPECからタスクを抽出し、セットアップ/共通/API別/結合に分類・順序付け
...
### タスク分解の原則
* 憲章遵守: `@agent_skills/jakarta-ee-standard/principles/` 配下の共通憲章（開発原則、品質基準、組織標準）を必ず遵守。プロジェクト固有の憲章がある場合は `{project_root}/principles/` も併せて遵守
* 抽象度の維持: タスクは「何を作るか」のみを記述。ソースコードや詳細な実装手順は記述しない
* アーキテクチャ適応: architecture_design.mdからアーキテクチャパターンを識別し、適切なタスクを分解・生成
  * マイクロサービス: 全エンティティ実装、楽観的ロック、2種類の検索、CORS対応
  * BFF: プロキシ+独自実装、外部API連携、JWT認証、一部エンティティ
