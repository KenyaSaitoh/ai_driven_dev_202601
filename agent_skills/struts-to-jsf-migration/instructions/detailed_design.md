...
10. セキュリティ要件（ソフトウェア実装に影響する項目のみ）
   * 「認証・認可の実装方式は以下でよろしいでしょうか？（Form認証、Container Managed Security、カスタム認証フィルター等）」
   * 「入力値バリデーションのルールは以下でよろしいでしょうか？（Bean Validation、カスタムバリデーター）」
   * 「SQLインジェクション対策として、PreparedStatementまたはJPQLのパラメータバインディングを使用します。他に必要な実装はありますか？」
   * 「XSS対策として、JSFの自動エスケープに加えて追加の実装は必要ですか？（カスタムコンバーター等）」
   * 「CSRF対策として、JSFの組み込み機能を使用します。追加の実装は必要ですか？」
   * 「パスワードのハッシュアルゴリズムはどれを実装しますか？（BCrypt、PBKDF2等、ストレッチング回数）」
   * 「個人情報や機密情報をログ出力する際、マスキング実装は必要ですか？」
   * 「アクセス制御の実装粒度は？（画面単位、ボタン単位、データ単位のフィルター実装）」

11. パフォーマンス要件（ソフトウェア実装に影響する項目のみ）
   * 「一覧表示の最大件数制限は何件ですか？（ビジネスロジックでの制御）」
   * 「ページネーションの実装は必要ですか？（1ページあたりの表示件数、Lazy Loading実装）」
   * 「検索結果が大量の場合、どのような実装をしますか？（JPQL LIMIT句、Stream API、カーソル等）」
   * 「重い処理の場合、非同期処理の実装は必要ですか？（@Asynchronous、CompletableFuture等）」
   * 「キャッシュの実装は必要ですか？（アプリケーションレベルのキャッシュ、@Cacheable相当の実装）」
   * 「ViewScopedのBean容量が大きくなる場合、データ保持方式をどう実装しますか？（Flash Scope、DTOの最小化等）」

12. データ整合性・トランザクション要件（ソフトウェア実装に影響する項目のみ）
   * 「トランザクション境界は以下でよろしいでしょうか？（@Transactionalの配置、Service層 vs Managed Bean層）」
   * 「ロールバック対象の例外は？（RuntimeException、checked Exception）」
   * 「同時更新制御の実装方式は？（楽観ロック @Version、悲観ロック SELECT FOR UPDATE、アプリケーションロック）」
   * 「データ削除の実装方式は？（論理削除、物理削除）」
   * 「エラーハンドリングの実装方針は？（Exception Mapper、エラーページ遷移、メッセージ表示）」
...
1. **複数の実装方法が考えられる場合**
   * 例: 画面スコープの選択（ViewScoped vs RequestScoped）
   * 例: データ受け渡し方式（Flash Scope vs Session Scope）

2. **SPECに明示されていない画面動作**
   * 例: バリデーションエラー時の画面表示方法
   * 例: 削除確認ダイアログの実装方式
   * 例: エラーメッセージの具体的な文言
...
4. **設定値・環境依存の情報**
   * 例: JNDI名（architecture_design.mdに記載がない場合）
   * 例: セッションタイムアウト値

5. **マイグレーション方針**
   * 例: Strutsの特定機能に対応するJSFの実装方式（複数の選択肢がある場合）

6. **セキュリティ実装に関する判断**（最優先で確認、ソフトウェア実装に影響するもののみ）
   * セキュリティ関連の実装方式は必ず人間が判断すべき
   * AIが推測で決定してはいけない項目
   * 例: 「管理者画面へのアクセス制御の実装方式は？（Filter、Interceptor、宣言的セキュリティ等）」
   * 例: 「パスワードハッシュの実装は？（BCrypt、PBKDF2等、ストレッチング回数）」
   * 例: 「個人情報を含むログ出力時、マスキング実装は必要ですか？」
   * 例: 「入力値バリデーションの実装は？（Bean Validation、カスタムバリデーター、サニタイゼーション）」

7. **パフォーマンス実装に関する判断**（必ず確認、ソフトウェア実装に影響するもののみ）
   * パフォーマンス要件は業務要件に依存するため、推測不可
   * 例: 「検索結果の最大件数は？それを超える場合の実装は？（LIMIT句、エラーメッセージ等）」
   * 例: 「商品マスタのキャッシュ実装は必要ですか？（アプリケーションレベルのキャッシュ実装方式）」
   * 例: 「大量データ処理の実装方式は？（ページネーション、Lazy Loading、バッチ処理等）」
   * 例: 「非同期処理の実装は必要ですか？（@Asynchronous、CompletableFuture等）」

8. **データ整合性・トランザクション実装に関する判断**（必ず確認、ソフトウェア実装に影響するもののみ）
   * データの重要度によって実装戦略が異なる
   * 例: 「在庫更新の同時実行制御の実装は？（楽観ロック @Version、悲観ロック、アプリケーションロック）」
   * 例: 「受注処理でエラーが発生した場合の実装は？（トランザクションロールバック、補償トランザクション）」
   * 例: 「データ削除の実装方式は？（論理削除フラグ、物理削除）」
...
2. **技術的な標準・常識**
   * 例: JSFの基本的なコンポーネント使用方法
   * 例: Bean Validationの標準的なアノテーション

3. **Agent Skillsルールで明示されている内容**
   * 例: StrutsからJSFへのマッピング規則
   * 例: アーキテクチャパターン、コーディング規約

4. **フレームワークのベストプラクティス**
   * 例: Managed Beanの標準的な実装パターン
   * 例: Faceletsの標準的なテンプレート構造

**バランスの取れた対話**:

* 「これは〜という理解で進めますが、以下の点のみ確認させてください」という形で、明確な点と不明確な点を分けて提示する
* 一度に多数の質問をするのではなく、本質的な判断ポイントに絞る
...
### 4.2 確認が必須の項目（ソフトウェア実装に直接影響するもののみ）

#### セキュリティ実装
1. **認証・認可の実装方式**
   - Form認証、Container Managed Security、カスタムフィルター/インターセプター等
   - ロールベースアクセス制御の実装粒度（画面単位、ボタン単位、データ単位）
   - アクセス制御ロジックの配置（Filter、Interceptor、@RolesAllowed相当）

2. **パスワードハッシュの実装**
   - ハッシュアルゴリズム（BCrypt、PBKDF2、Argon2等）
   - ストレッチング回数（BCryptのコストファクター等）
   - ソルトの生成方式

3. **入力値検証の実装**
   - Bean Validationアノテーション（@NotNull、@Size、@Pattern等）
   - カスタムバリデーターの実装要否
   - サニタイゼーション処理の実装（XSS対策）
   - 必須チェック、形式チェック、範囲チェックのルール

4. **個人情報・機密情報の取り扱い実装**
   - ログ出力時のマスキング実装（Log4j PatternLayout、カスタムAppender等）
   - 画面表示時のマスキング実装（JSFコンバーター、カスタムコンポーネント等）
   - データベース保存時の暗号化実装（JPA AttributeConverter等）
...
6. **XSS/CSRF対策の実装**
   - JSFの自動エスケープ機能の活用
   - カスタムエスケープ処理の実装要否
   - CSRFトークン検証の実装（JSF組み込み機能、カスタム実装）

#### パフォーマンス実装
1. **データ取得件数の制御実装**
   - 一覧表示の最大件数（ビジネスロジックでの制限）
   - JPQL/SQL での LIMIT句の実装
   - 最大件数を超える場合の実装（エラーメッセージ、警告表示等）

2. **ページネーションの実装**
   - 実装方式（JPQL LIMIT/OFFSET、Criteria API setFirstResult/setMaxResults）
   - ページサイズの決定（デフォルト値）
   - ページング情報の保持方式（Managed Bean、Session等）

3. **Lazy Loading（遅延ロード）の実装**
   - JPAの@OneToMany(fetch = FetchType.LAZY)の活用
   - JSF Lazy DataModelの実装
   - 必要なデータのみを取得するクエリ設計

4. **キャッシュの実装**
   - アプリケーションレベルのキャッシュ実装（Map、ConcurrentHashMap等）
   - キャッシュクリア処理の実装タイミング
   - Managed Beanスコープでのキャッシュ活用（@ApplicationScoped等）
...
#### データ整合性・トランザクション実装
1. **トランザクション境界の実装**
   - @Transactionalアノテーションの配置（Service層、Managed Bean層）
   - トランザクション伝播（REQUIRED、REQUIRES_NEW等）の選択
   - ロールバック対象の例外設定（RuntimeException、checked Exception）

2. **同時更新制御の実装**
   - 楽観ロック（@Versionアノテーション）の実装
   - 悲観ロック（JPQLのSELECT FOR UPDATE）の実装
   - アプリケーションレベルのロック実装

3. **データ削除の実装方式**
   - 論理削除の実装（削除フラグ、@Where、@SQLDelete）
...
### セキュリティ実装
1. パスワードハッシュの実装
   - 実装方式の選択肢: BCrypt（推奨）/ PBKDF2 / Argon2
   - 推奨実装: BCrypt（コストファクター10〜12）
   - 確認事項: どのハッシュアルゴリズムを実装しますか？ストレッチング回数は？

2. 個人情報のログ出力マスキング実装
   - 実装方式の選択肢: カスタムAppender実装 / PatternLayoutのコンバーター / ログ出力前の手動マスキング
   - 推奨実装: PatternLayoutのカスタムコンバーター
   - 確認事項: ログ出力時のマスキング処理をどのレイヤーで実装しますか？

3. アクセス制御の実装
   - 実装方式の選択肢: カスタムFilter / Interceptor / Managed Beanでのチェック
   - 推奨実装: Filter（横断的関心事として実装）
   - 確認事項: アクセス制御ロジックをどこに実装しますか？画面単位/機能単位/データ単位のどの粒度ですか？
```
...
1. 検索結果の最大件数制限実装
   ...
   - 推奨実装: JPQL LIMIT句（データベースレベルで制限）
   - 確認事項: 検索結果は最大何件まで表示しますか？それを超える場合、どのような実装をしますか？（エラーメッセージ、警告表示等）

2. ページネーションの実装
   - 実装方式の選択肢: JPQL LIMIT/OFFSET / Criteria API / JSF Lazy DataModel
   - 推奨実装: JPQL LIMIT/OFFSET（シンプルな実装）
   - 確認事項: ページネーションは必要ですか？1ページあたりの表示件数は？

3. キャッシュの実装
   - 実装方式の選択肢: @ApplicationScoped Beanでのキャッシュ / Mapを使った手動実装 / キャッシュなし
   - 推奨実装: @ApplicationScoped Beanでのキャッシュ（更新頻度が低いマスタデータ）
   - 確認事項: マスタデータのキャッシュ実装は必要ですか？キャッシュクリアのタイミングは？
```
...
### データ整合性・トランザクション実装
1. 同時更新制御の実装
   - 実装方式の選択肢: 楽観ロック（@Version） / 悲観ロック（SELECT FOR UPDATE） / アプリケーションロック
   - 推奨実装: 楽観ロック（競合が少ない場合）、悲観ロック（競合が多い場合）
   - 確認事項: 在庫更新などの同時実行制御はどのように実装しますか？

2. データ削除の実装方式
   - 実装方式の選択肢: 論理削除（削除フラグ、@Where/@SQLDelete実装） / 物理削除（EntityManager.remove）
   - 推奨実装: 論理削除（監査要件がある場合）
   - 確認事項: データ削除はどのように実装しますか？論理削除の場合、削除フラグ名は？

3. トランザクション境界の実装
   - 実装方式の選択肢: Service層に@Transactional / Managed Bean層に@Transactional
   - 推奨実装: Service層に@Transactional（ビジネスロジック単位でトランザクション管理）
   - 確認事項: トランザクション境界はどこに実装しますか？ロールバック対象の例外は？
```
...
推測や仮定で詳細設計を作成しないでください。

**質問すべき典型的なケース**:

1. **画面動作の詳細**
   * 画面の動作がSPECから一意に決まらない場合
   * 例: 「削除ボタン押下時、確認ダイアログを表示しますか？」
...
3. **エラーハンドリングの方針**
   * エラー時の具体的な処理方法が不明な場合
   * 例: 「DB接続エラー時、エラーページに遷移しますか？それとも画面上にメッセージ表示しますか？」

4. **画面遷移の詳細**
   * 遷移条件や遷移先が曖昧な場合
   * 例: 「登録成功後、一覧画面に戻りますか？それとも詳細画面を表示しますか？」
...
1. **SPECから明らかな内容**
   * screen_design.mdに定義された画面レイアウト → XHTML設計は自動的に導出
   * data_model.mdに定義されたテーブル構造 → JPAエンティティ設計は自動的に導出

2. **技術標準・ベストプラクティス**
   * JSFの標準的な実装パターン
   * Bean Validationの標準的な使用方法
   * 標準的な命名規則

3. **Agent Skillsルールで定義済み**
   * StrutsからJSFへのマッピング規則
   * アーキテクチャパターン
   * コーディング規約
