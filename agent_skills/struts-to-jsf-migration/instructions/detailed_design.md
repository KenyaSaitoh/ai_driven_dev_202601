...
* `{spec_directory}/detailed_design/detailed_design/FUNC_XXX/behaviors.md` - 単体テスト用（Gherkin 記法。@agent_skills/struts-to-jsf-migration/principles/common_rules.md の「振る舞いの記法（Gherkin）」を参照。種別は上記「behaviors.mdの種別」参照）

behaviors.mdの種別（本インストラクションで参照する区別）:
* basic_design/behaviors.md: 結合・E2Eテスト用（システム全体、画面間遷移、実際のDB・画面レンダリング、Playwright等）
* detailed_design/detailed_design/FUNC_XXX/behaviors.md: 単体テスト用（1メソッド単位、依存関係はモック。本フェーズで新規作成）
...
以下の観点で、SPECに明記されていない場合はユーザーに質問する。具体的な確認例は2.3を参照。

1. パッケージ構造・実装するクラス（Managed Bean、Service、Entity、DTO）
2. Managed Bean設計（スコープ、プロパティ、アクションメソッド）
3. 画面レイアウト（表示項目、テーブル・フォーム・ボタン配置）
4. バリデーション（Bean Validation、カスタムバリデーション）
5. 画面遷移（遷移先、Flash Scope等でのデータ受け渡し）
6. ビジネスロジック（計算ロジック、状態遷移）
7. エラーハンドリング（想定されるエラーシナリオ）
8. セッション管理（Flash Scope、Session Scope、リクエストパラメータ）
9. データソース設定（JNDI名、architecture_design.mdまたはpersistence.xmlから確認）
10. セキュリティ要件（認証・認可、入力検証、SQL/XSS/CSRF対策、パスワードハッシュ、ログマスキング、アクセス制御）
11. パフォーマンス要件（最大件数、ページネーション、大量データ、非同期、キャッシュ、ViewScopedのデータ保持）
12. データ整合性・トランザクション（境界、ロールバック、同時更新制御、論理/物理削除、エラーハンドリング）
...
## 4. 非機能要件の確認（セキュリティ・パフォーマンス・データ整合性）

セキュリティ・パフォーマンス・データ整合性に関する実装判断で、SPECに明記されていないものは必ずユーザーに確認する。AIは推測で決定しない。

* 確認すべき観点・カテゴリ: @agent_skills/struts-to-jsf-migration/principles/architecture.md の「11.5 非機能要件の確認原則（詳細設計時）」を参照
