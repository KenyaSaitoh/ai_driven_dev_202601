...
ドメインフォルダ名の命名は以下の規約に従う:

* ドメイン名は小文字のスネークケースを使用する（例: common, orders, books_proxy）
* ハイフン（`-`）は使用しない
* 固定ドメイン: common（最優先で実装する共通ドメイン）
* 可変ドメイン: プロジェクト固有のドメイン名（例: orders, books_proxy, images等）

根拠: 一貫した命名により、フォルダ構成の可読性とツール連携の安定性を確保する
