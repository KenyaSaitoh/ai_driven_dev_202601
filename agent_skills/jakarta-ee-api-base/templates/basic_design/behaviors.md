...
本文書は、[PROJECT_NAME]システム全体のE2Eテスト用の振る舞い、受入基準を記述する。システム全体のE2Eテストシナリオ、複数コンポーネント連携、実際のDB/外部APIを使用した結合テストの受入基準を定義する。

注意: 
* タスク単位の単体テストシナリオは、各detailed_design/{target}/behaviors.mdを参照すること（templates/detailed_design/behaviors.mdから生成）
* このbehaviors.mdはE2Eテスト専用です
