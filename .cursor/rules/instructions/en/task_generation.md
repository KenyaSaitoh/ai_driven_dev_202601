...
## Project Role (Example: berry-books-api)

**BFF Pattern for berry-books-api:**

- **Architecture Pattern**: BFF (Backend for Frontend)
- **Role**: Single entry point for frontend (berry-books-spa)
- **External API Integration**: 
  - back-office-api: Book, inventory, and category management
  - customer-hub-api: Customer management
- **Implementation Patterns**:
  - **Proxy**: BookResource, CategoryResource (transparent forwarding to external APIs)
  - **Local Implementation**: AuthResource (JWT authentication), OrderResource (order processing)
- **Managed Data**: Only order data (ORDER_TRAN, ORDER_DETAIL) directly managed
- **Important Notes**: 
  - Do NOT implement Book, Stock, Category, Publisher, Customer entities (managed by external APIs)
  - Access book/inventory information via external API (BackOfficeRestClient)

**Note:** The above is an example for berry-books-api. Determine your project's role from the specifications (`{spec_directory}/system/architecture_design.md`).
...
- **Order Entities**: OrderTran, OrderDetail, OrderDetailPK (only these implemented)
- **Common Services**: DeliveryFeeService (delivery fee calculation)
- **JWT Authentication Infrastructure**: JwtUtil, JwtAuthenFilter, AuthenContext
- **External API Integration**: BackOfficeRestClient, CustomerHubRestClient
- **Common DTOs**: ErrorResponse, DTOs for external APIs (BookTO, StockTO, CustomerTO, etc.)
- **Common Utilities**: MessageUtil, AddressUtil, SettlementType

**Note - BFF Pattern Constraints**: 
The following components are NOT implemented (managed by external APIs):
- ❌ Book, Stock, Category, Publisher, Customer entities
