...
詳細設計書から本番コード（`src/main/`）を生成します。**重要**: commonドメインを最優先で実装。
...
* spec_directory: projects/sdd-wf/bookstore/back-office-api/specs/baseline
* target_domain: common
* skip_infrastructure: true  # 初回のみ: 開発環境構築済みの場合
```

他ドメイン（books, categories, publishers, stocks, workflows）の実装は、target_domainを変更して同様に実行。`skip_infrastructure`不要。
