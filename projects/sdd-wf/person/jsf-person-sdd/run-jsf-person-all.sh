...
# 2. Payara Server の起動
# 3. データソースのセットアップ
# 4. jsf-person-sdd のDB初期化、WAR化、デプロイ
###############################################################################
...
LOG_FILE="$LOG_DIR/run-jsf-person-all-sdd_${TIMESTAMP}.log"
...
# ステップ4: jsf-person-sdd のセットアップとデプロイ
log "STEP 4: jsf-person-sdd のセットアップとデプロイ..."
log_info "  -> データベーステーブルを作成中..."
./gradlew :jsf-person-sdd:setupHsqldb >> "$LOG_FILE" 2>&1 || error_exit "jsf-person-sdd の DB セットアップに失敗"
log_info "  -> WAR ファイルをビルド中..."
./gradlew :jsf-person-sdd:war >> "$LOG_FILE" 2>&1 || error_exit "jsf-person-sdd のビルドに失敗"
log_info "  -> デプロイ中..."
./gradlew :jsf-person-sdd:deploy >> "$LOG_FILE" 2>&1 || error_exit "jsf-person-sdd のデプロイに失敗"
log "✓ jsf-person-sdd のデプロイが完了しました"
echo ""
...
log "  - jsf-person-sdd: http://localhost:8080/jsf-person-sdd/faces/PersonTablePage.xhtml"
echo ""
log "${YELLOW}■ 停止方法${NC}"
log "  アプリケーション:"
log "    ./gradlew :jsf-person-sdd:undeploy"
log ""
log "  サーバー:"
