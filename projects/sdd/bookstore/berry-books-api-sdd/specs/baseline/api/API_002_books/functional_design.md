...
```
GET /api/books/search/jpql?categoryId={id}&keyword={keyword}
```
...
### 3.4 書籍検索（Criteria API版）

#### 3.4.1 エンドポイント

```
GET /api/books/search/criteria?categoryId={id}&keyword={keyword}
```
...
|-----|--------|---------------|
| 実装方式 | JPQL文字列クエリ | Criteria APIの型安全なクエリ |
| パフォーマンス | 同等 | 同等 |
| 動的条件 | 文字列結合 | プログラマティック |
| コンパイル時チェック | なし | あり（型安全） |

#### 3.4.6 ビジネスルール

| ルールID | 説明 |
|---------|-------------|
| BR-BOOK-001 | キーワード検索は書籍名、著者名で部分一致 |
| BR-BOOK-002 | カテゴリIDが0の場合は全カテゴリを対象 |
| BR-BOOK-003 | 検索結果には在庫情報（Stock）を含む |
