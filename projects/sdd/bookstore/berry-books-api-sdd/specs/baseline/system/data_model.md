...
Book → Stock → Book という双方向の関係により、JSON-Bシリアル化時に無限ループが発生する可能性があるため、`Stock.book`フィールドに`@JsonbTransient`アノテーションを付与してJSONシリアル化から除外します。
...
@Entity
public class OrderTran {
    @OneToMany(mappedBy = "orderTran", cascade = CascadeType.ALL)
...
```java
@Query("SELECT new pro.kensait.berrybooks.service.order.OrderHistoryTO(" +
...
List<OrderHistoryTO> getOrderHistory2(@Param("customerId") Integer customerId);
```
...
```sql
SELECT * FROM ORDER_TRAN WHERE CUSTOMER_ID = ? ORDER BY ORDER_DATE DESC
```

**インデックス**: IDX_CUSTOMER_ID
