...
## 3. 注文履歴取得 (`GE/api/orders/history`)
...
## 5. 注文明細取得 (`GE/api/orders/{tranId}/details/{detailId}`)
...
| シナリオID | API | 受入基準 |
|-----------|-----|---------|
| PERF-001 | POS/api/orders | 500ms以内（95パーセンタイル） |
| PERF-002 | GE/api/orders/history | 500ms以内（95パーセンタイル） |
