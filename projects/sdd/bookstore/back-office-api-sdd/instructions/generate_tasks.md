...
- SPEC配置ディレクトリ: `projects\java\berry-books-api-sdd\specs`
- 出力先: `projects\java\berry-books-api-sdd\tasks`
...
- 複数機能で共有される共通コンポーネント
- 共通エンティティ（例：Publisher, Category, Book, Stock, Customer等）
- 共通サービス（例：DeliveryFeeService等）
- 共通ユーティリティ（例：バリデーション、フォーマッター等）
- JWT認証基盤（JwtUtil, JwtAuthenticationFilter等）
- 共通DTO/レスポンスモデル
