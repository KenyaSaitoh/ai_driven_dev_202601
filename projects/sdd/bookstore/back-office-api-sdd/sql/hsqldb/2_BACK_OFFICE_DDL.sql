...
WORKFLOW_TYPE    VARCHAR(30) NOT NULL,                                -- ワークフロー種別 (ADD_NEW_BOOK, REMOVE_BOOK, ADJUST_BOOK_PRICE)

-- 現在の状態（この操作の結果の状態）
STATE            VARCHAR(20) NOT NULL,                                -- 状態 (NEW, APPLIED, APPROVED)

-- 対象書籍情報（REMOVE_BOOK、ADJUST_BOOK_PRICEの場合に使用）
BOOK_ID          INT,                                                 -- 対象書籍ID
...
-- 価格情報（ADD_NEW_BOOKの場合は新規価格、ADJUST_BOOK_PRICEの場合は改定後価格）
PRICE            INT,                                                 -- 価格
