...
| シナリオID | 説明 | Given（前提条件） | When（操作） | Then（期待結果） |
|-----------|------|----------------|------------|---------------|
| US-STOCK-001 | 在庫数を更新できる | bookId=1, quantity=10, version=1 | PUT /api/stocks/1<br/>{version: 1, quantity: 15} | 在庫数が15に更新<br/>version=2 |
...
| シナリオID | 説明 | Given（前提条件） | When（操作） | Then（期待結果） |
|-----------|------|----------------|------------|---------------|
| US-WF-001 | 一般社員が新規書籍追加を申請できる | operatedBy=1 (ASSOCIATE) | POST /api/workflows<br/>{workflowType: "ADD_NEW_BOOK"}<br/>POST /api/workflows/1/apply | ワークフロー作成・申請成功<br/>state: "APPLIED" |
| US-WF-002 | 管理職が承認できる | operatedBy=2 (MANAGER)<br/>state=APPLIED | POST /api/workflows/1/approve | 承認成功<br/>書籍マスタに反映<br/>state: "APPROVED" |
...
| シナリオID | 説明 | ステップ | 受入基準 |
|-----------|------|---------|---------|
| E2E-001 | ログインから書籍検索まで | 1. POST /api/auth/login<br/>2. GET /api/books<br/>3. GET /api/books/1 | 全ステップが成功<br/>JWT Cookieが維持される |
| E2E-002 | ワークフロー作成から承認まで | 1. POST /api/workflows<br/>2. POST /api/workflows/1/apply<br/>3. POST /api/workflows/1/approve<br/>4. GET /api/books (新規書籍確認) | 全ステップが成功<br/>新規書籍が反映される |
| E2E-003 | 在庫更新フロー | 1. GET /api/stocks/1<br/>2. PUT /api/stocks/1<br/>3. GET /api/books/1 (在庫確認) | 全ステップが成功<br/>在庫数が正しく更新される |
