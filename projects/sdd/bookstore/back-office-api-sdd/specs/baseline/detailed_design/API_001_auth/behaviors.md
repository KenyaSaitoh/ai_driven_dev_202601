...
| シナリオID | API | 受入基準 |
|-----------|-----|---------|
| PERF-001 | POS/api/auth/login | 500ms以内（95パーセンタイル） |
| PERF-002 | POS/api/auth/logout | 50ms以内（95パーセンタイル） |
| PERF-003 | GE/api/auth/me | 500ms以内（95パーセンタイル）（将来実装時） |
