...
## 4. 書籍検索（デフォルト） (`GE/api/books/search`)
...
## 5. 書籍検索（JPQL） (`GE/api/books/search/jpql`)
...
## 6. 書籍検索（Criteria API） (`GE/api/books/search/criteria`)
...
## 7. カテゴリ一覧取得 (`GE/api/books/categories`)
...
| PERF-002 | GE/api/books/{id} | 500ms以内（95パーセンタイル） |
| PERF-003 | GE/api/books/search | 500ms以内（95パーセンタイル） |
| PERF-004 | GE/api/books/search/jpql | 500ms以内（95パーセンタイル） |
| PERF-005 | GE/api/books/search/criteria | 500ms以内（95パーセンタイル） |
...
| シナリオID | 説明 | Given（前提条件） | When（操作） | Then（期待結果） |
|-----------|------|----------------|------------|---------------|
| CONC-001 | 複数ユーザーが同時に書籍一覧を取得できる | - | 3ユーザーが同/api/books | 全リクエストが成功<br/>独立したトランザクション |
| CONC-002 | 読み取り専用のため競合なし | - | 同/api/books | データ競合なし<br/>ロック不要 |
