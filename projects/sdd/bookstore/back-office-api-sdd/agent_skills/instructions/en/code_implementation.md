...
## Microservice Pattern Specific Implementation Requirements

### back-office-api Architecture Characteristics

**Microservice Role:**
- Complete data management service for books, inventory, categories, and publishers
- Called as REST API by BFF (berry-books-api)
- Manages independent database
- CORS-enabled for cross-origin requests

### All Entities Implementation
...
- ✅ BookService (two search methods)
- ✅ StockService (optimistic locking handling)
- ✅ CategoryService
- ✅ PublisherService

**Implement All Resources:**
- ✅ BookResource (two search endpoints)
- ✅ StockResource (optimistic locking support)
- ✅ CategoryResource
- ✅ PublisherResource

### Optimistic Locking Implementation

**Stock Entity MUST have @Version annotation** for concurrent inventory updates:
...
**OptimisticLockExceptionMapper** returns HTTP 409 Conflict:
...
public class OptimisticLockExceptionMapper 
    ...
    public Response toResponse(OptimisticLockException exception) {
        return Response.status(Response.Status.CONFLICT)
            .entity(new ErrorResponse("Stock modified by another transaction"))
            .build();
    }
}
```
...
**Implement TWO search methods for books:**

1. **JPQL Search (BookDao)**: Dynamic JPQL query
   - Endpoint: `GET /books/search/jpql`
   
2. **Criteria API Search (BookDaoCriteria)**: Type-safe Criteria API
   - Endpoint: `GET /books/search/criteria`

Both implementations return the same results and are exposed as separate endpoints.

### CORS Configuration

**CORSFilter** to accept cross-origin requests from BFF:
...
1. **Unit Tests**: Test business logic with mocked DAOs
2. **Concurrency Tests**: Test concurrent stock updates (IMPORTANT!)
3. **Search Comparison Tests**: Verify JPQL and Criteria API return same results
4. **Integration Tests**: Test REST APIs, verify HTTP 409 Conflict for version mismatch
