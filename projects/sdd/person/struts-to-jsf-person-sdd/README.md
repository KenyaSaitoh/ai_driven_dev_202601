# struts-to-jsf-person-sdd プロジェクト
...
> StrutsからJakarta EE（JSF）への段階的な移行プロセスを、AIを活用して学習できます。

> **SDDによる移行手法:**
> - レガシーシステム（Struts 1.3）を現代的なフレームワーク（Jakarta EE）へ移行
> - AIとの対話を通じて、段階的に移行計画を立て、実装を進める
> - 既存のビジネスロジックを保持しながら、プレゼンテーション層を刷新
...
- **トップページ**: http://localhost:8088/struts-to-jsf-person-sdd/

## 🎯 プロジェクト構成

```
projects/sdd/bookstore/struts-to-jsf-person-sdd/
├── src/
│   ├── main/
│   │   ├── java/
│   │   │   └── dev/
