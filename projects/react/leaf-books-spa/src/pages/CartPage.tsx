...
const CartPage: React.FC = () => {
  ...
  const settlementType = 1; // 決済タイプ（1: クレジットカードなど）
  ...
  const handleOrder = async () => {
    ...
    if (window.confirm('この内容で注文しますか？')) {
      ...
      try {
        ...
        clearCart();
        alert(`注文が完了しました（注文番号: ${orderResponse.orderTranId}）`);
      ...
      {items.length === 0 ? (
        ...
        <>
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <button onClick={handleOrder} className="btn-primary" disabled={ordering}>
              {ordering ? '注文処理中...' : '買い物を終了し注文する'}
            </button>
          </div>
        </>
      )}
