...
* Node.js 16+ / npm 8+
* バックエンドAPI (`back-office-api-sdd-agile`) が起動している必要があります

### 依存関係のインストール

```bash
cd projects/sdd-wf/bookstore/back-office-spa
npm install
```
...
export default defineConfig({
  server: {
    ...
    proxy: {
      '/api': {
        target: 'http://localhost:8080/back-office-api-sdd-agile',
        changeOrigin: true,
      }
    }
  }
...
* **ベースURL**: `http://localhost:8080/back-office-api-sdd-agile`
* **認証**: Cookie（JWT）による認証
* **認証エンドポイント**:
