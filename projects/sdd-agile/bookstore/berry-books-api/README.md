...
common（data_model, external_interface, architecture_design）を先に整え、各ユースケースに userstory.md / behaviors.md を配置します。OpenAPIは `common/openapi/` に格納。
...
│   │   │   └── openapi/            # OpenAPI YAML（back-office-api 連携用）
│   │   ├── usecases/               # ユースケース別
