# berry-books-api-sdd-agile - データモデル仕様書
...
* 注意: 上記のER図はberry-books-api-sdd-agileが管理するテーブルのみを示しています。書籍・在庫・カテゴリ・出版社データはback-office-api-sdd-agile、顧客データはcustomer-hub-apiが管理します。

### 2.2 他のサービスのテーブル構造（参考）

以下は参考情報です。これらのテーブルはberry-books-api-sdd-agileでは管理されません。

* back-office-api-sdd-agile管理テーブル:
```mermaid
erDiagram
...
* 外部キー:
  ...
  * BOOK_ID: 外部キー制約なし（書籍データはback-office-api-sdd-agileが管理）
...
#### 5.1.1 berry-books-api-sdd-agile管理テーブル（本サービス）
...
#### 6.1.1 berry-books-api-sdd-agile管理テーブル
