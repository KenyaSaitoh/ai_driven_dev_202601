...
public class CustomerHubRestClient {
    private static final Logger logger = LoggerFactory.getLogger(
            CustomerHubRestClient.class);

    private static final String PROPERTY_KEY = "customer-hub-api.base-url";
    private static final String ENV_VAR_KEY = "CUSTOMER_HUB_API_BASE_URL";
    private static final String DEFAULT_URL = "http://localhost:8080/customer-hub-api/customers";
    ...
    public CustomerTO findByEmail(String email) {
        ...
        try (Response response = target.request(MediaType.APPLICATION_JSON).get()) {
            return switch (response.getStatus()) {
                case 200 -> response.readEntity(CustomerTO.class);
                case 404 -> {
                    // 顧客が見つからない場合
    ...
    public CustomerTO register(CustomerTO customerTO) {
        logger.info("[ CustomerHubRestClient#register ] customer=" + customerTO);
        ...
        try (Response response = target.request(MediaType.APPLICATION_JSON)
            ...
            return switch (response.getStatus()) {
                ...
                case 409 -> {
                    ...
                    throw new EmailAlreadyExistsException(customerTO.email(), error.message());
                }
                default -> {
                    // その他のエラー
                    logger.error("Unexpected response status: " + response.getStatus());
