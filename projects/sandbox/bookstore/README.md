# Bookstore Projects - Sandbox（プロンプトエンジニアリング用）

## 📖 概要

書店ドメインの完成版プロジェクト集です。**master からコピーした完成版**で、様々なプロンプトエンジニアリングをトライアルするためのサンドボックスです。master は模範解答として手を入れない想定ですが、こちらは自由に改変・試行できます。

> **Sandbox**: このディレクトリは `projects/sandbox/bookstore/` に配置されています。  
> **Gradle プロジェクト名**: ビルド・デプロイ時は `-sandbox` 付きの名前を使用します（`berry-books-api-sandbox`, `back-office-api-sandbox`, `customer-hub-api-sandbox`）。コンテキストルートは `/berry-books-api-sandbox`, `/back-office-api-sandbox`, `/customer-hub-api-sandbox` です。
...
> **Note**: sandbox には customer-hub-swing は含まれていません（master のみ）。
...
```bash
# プロジェクトルートで実行（sandbox 用は -sandbox 付き）
./gradlew :berry-books-api-sandbox:setupHsqldb      # 注文管理テーブル
./gradlew :back-office-api-sandbox:setupHsqldb     # 書籍・在庫テーブル
./gradlew :customer-hub-api-sandbox:setupHsqldb    # 顧客テーブル
```
...
./gradlew :berry-books-api-sandbox:war
./gradlew :back-office-api-sandbox:war
./gradlew :customer-hub-api-sandbox:war
```
...
```bash
# プロジェクトルートで実行（sandbox 用）
./gradlew :berry-books-api-sandbox:deploy
./gradlew :back-office-api-sandbox:deploy
./gradlew :customer-hub-api-sandbox:deploy
```

### ⑦ アプリケーション作成・更新のたびに実行

```bash
# アプリケーションを再ビルドして再デプロイ（sandbox 用）
# 例：berry-books-api-sandbox の場合
./gradlew :berry-books-api-sandbox:war
./gradlew :berry-books-api-sandbox:deploy

# 例：back-office-api-sandbox の場合
./gradlew :back-office-api-sandbox:war
./gradlew :back-office-api-sandbox:deploy

# 例：customer-hub-api-sandbox の場合
./gradlew :customer-hub-api-sandbox:war
./gradlew :customer-hub-api-sandbox:deploy
```
...
```bash
# プロジェクトルートで実行（sandbox 用）
./gradlew :berry-books-api-sandbox:undeploy
./gradlew :back-office-api-sandbox:undeploy
./gradlew :customer-hub-api-sandbox:undeploy
```
