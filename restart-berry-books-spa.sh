...
###############################################################################
# Berry Books SPA Restart Script
# 
# SPAを再起動するスクリプトです
#
# 使用方法:
#   ./restart-berry-books-spa.sh           # 通常の再起動
#   ./restart-berry-books-spa.sh --clean   # Viteキャッシュクリア + 再起動
#   ./restart-berry-books-spa.sh --rebuild # 完全リビルド (node_modules削除 + npm install + 再起動)
###############################################################################
...
if [ "$FULL_REBUILD" = true ]; then
    ...
    rm -rf "$SCRIPT_DIR/projects/master/bookstore/berry-books-spa/node_modules/.vite"
    
    log_info "  -> back-office-spa のキャッシュをクリア中..."
    rm -rf "$SCRIPT_DIR/projects/master/bookstore/back-office-spa/node_modules/.vite"
    
    log_info "  -> customer-hub-spa のキャッシュをクリア中..."
    rm -rf "$SCRIPT_DIR/projects/master/bookstore/customer-hub-spa/node_modules/.vite"
...
log "  通常再起動:           ./restart-berry-books-spa.sh"
log "  キャッシュクリア:     ./restart-berry-books-spa.sh --clean"
log "  完全リビルド:         ./restart-berry-books-spa.sh --rebuild"
echo ""
log "=============================================="
